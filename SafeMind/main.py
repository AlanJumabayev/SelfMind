...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QTabWidget, QWidget, QVBoxLayout, 
                           ...
                           QAbstractItemDelegate, QInputDialog)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        if PDF_PROCESSOR_AVAILABLE:
            self.pdf_processor = PDFProcessor(self.anonymizer)
        else:
    ...
    def setup_pdf_tab(self):
        ...
        if self.pdf_processor:
            self.extract_text_button = QPushButton("Извлечь текст")
            self.extract_text_button.clicked.connect(self.on_extract_text_pdf)
            self.extract_text_button.setEnabled(False)
            pdf_buttons_layout.addWidget(self.extract_text_button)
            
            self.extract_images_button = QPushButton("Извлечь изображения")
            self.extract_images_button.clicked.connect(self.on_extract_images_pdf)
            self.extract_images_button.setEnabled(False)
            pdf_buttons_layout.addWidget(self.extract_images_button)
            
            self.add_watermark_button = QPushButton("Добавить водяной знак")
            self.add_watermark_button.clicked.connect(self.on_add_watermark_pdf)
            self.add_watermark_button.setEnabled(False)
            pdf_buttons_layout.addWidget(self.add_watermark_button)
    ...
    def on_clear_pdf_tab(self):
        ...
        if hasattr(self, 'extract_text_button'):
            self.extract_text_button.setEnabled(False)
        if hasattr(self, 'extract_images_button'):
            self.extract_images_button.setEnabled(False)
        if hasattr(self, 'add_watermark_button'):
            self.add_watermark_button.setEnabled(False)
        ...
        if file_path:
            try:
                ...
                if pdf_document.page_count > 0:
                    ...
                    if hasattr(self, 'add_watermark_button'):
                        self.add_watermark_button.setEnabled(True)
    ...
    def on_anonymize_pdf(self):
        ...
        if self.pdf_processor:
            self.worker = WorkerThread(
                self.pdf_processor.anonymize_pdf,
    ...
    def on_extract_text_pdf(self):
        if not self.current_pdf_path or not self.pdf_processor:
            return
            
        self.progress_bar.setVisible(True)
        
        self.worker = WorkerThread(
            self.pdf_processor.extract_text_from_pdf,
            self.current_pdf_path
        )
        self.worker.progress.connect(self.update_progress)
        self.worker.finished.connect(self.on_text_extracted)
        self.worker.start()
        
    def on_text_extracted(self, result, message):
        self.progress_bar.setVisible(False)
        
        if result:
            
            text_path = self.current_pdf_path.replace('.pdf', '.txt')
            try:
                ...
                reply = QMessageBox.information(
                    self, 
                    "Текст извлечен", 
                    f"Текст извлечен и сохранен в файл {text_path}\nОткрыть файл?",
                ...
                if reply == QMessageBox.Yes:
                    try:
                        if os.name == 'nt':  
                            ...
                            subprocess.call(['xdg-open', text_path])
                    except Exception as e:
                        QMessageBox.warning(self, "Предупреждение", f"Не удалось открыть файл: {str(e)}")
    ...
    def on_extract_images_pdf(self):
        if not self.current_pdf_path or not self.pdf_processor:
            return
            
        
        output_dir = os.path.splitext(self.current_pdf_path)[0] + "_images"
        
        self.progress_bar.setVisible(True)
        
        self.worker = WorkerThread(
            self.pdf_processor.extract_images,
            self.current_pdf_path,
            output_dir
        )
        self.worker.progress.connect(self.update_progress)
        self.worker.finished.connect(self.on_images_extracted)
        self.worker.start()
        
    def on_images_extracted(self, result, message):
        self.progress_bar.setVisible(False)
            ...
            if num_images > 0:
                ...
                if reply == QMessageBox.Yes:
                    ...
                    try:
                        ...
                        QMessageBox.warning(self, "Предупреждение", f"Не удалось открыть папку: {str(e)}")
                        
            self.statusBar().showMessage(message)
        else:
            QMessageBox.warning(self, "Ошибка", message)
            
    def on_add_watermark_pdf(self):
        if not self.current_pdf_path or not self.pdf_processor:
            return
            
        text, ok = QInputDialog.getText(self, "Водяной знак", "Введите текст водяного знака:", QLineEdit.Normal, "КОНФИДЕНЦИАЛЬНО")
        if ok and text:
            self.progress_bar.setVisible(True)
            
            self.worker = WorkerThread(
                self.pdf_processor.add_watermark,
                self.current_pdf_path,
                text
            )
            self.worker.progress.connect(self.update_progress)
            self.worker.finished.connect(self.on_watermark_added)
            self.worker.start()
            
    def on_watermark_added(self, result, message):
        self.progress_bar.setVisible(False)
